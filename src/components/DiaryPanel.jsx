...
function DiaryEntry({ entry }) {
  return (
    <div className={styles.entry}>
      <div className={styles.meta}>{AGE_LABELS[entry.age] || entry.age}</div>
      <div className={styles.desc}>{entry.event.description}</div>
      <div className={styles.choice}>"{entry.choice.text}"</div>
    ...
    <div className={styles.col}>
      ...
      <div className={styles.scroll} ref={scrollRef}>
        {history.length === 0 ? (
          ...
          history.map((entry, i) => <DiaryEntry key={i} entry={entry} />)
        )}
      </div>
    </div>
  )
